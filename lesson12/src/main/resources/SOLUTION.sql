delete from PAYMENT where PAYMENT.STUDENT_ID in (select id from STUDENT where id in (select STUDENT_ID from MARK where MARK > 4));
...
SELECT DISTINCT s.id, s.name FROM Student s JOIN Mark m ON s.id = m.student_id WHERE m.mark < 4;
...
delete from PAYMENT where PAYMENT.TYPE_ID in (select ID from PAYMENTTYPE where NAME = 'DAILY');
